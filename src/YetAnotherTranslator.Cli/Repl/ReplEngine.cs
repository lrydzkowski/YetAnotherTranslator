...
internal class ReplEngine
{
    ...
    private async Task<bool> HandleCommandAsync(Command command, CancellationToken cancellationToken)
    {
        bool shouldQuit = command.Type switch
        {
            ...
            CommandType.TranslateWordAutodetect => await HandleTranslateWordCommandAsync(command, cancellationToken),
            CommandType.TranslateWordToEnglish => await HandleTranslateWordCommandAsync(command, cancellationToken),
            CommandType.TranslateWordToPolish => await HandleTranslateWordCommandAsync(command, cancellationToken),
            CommandType.TranslateTextAutodetect => await HandleTranslateTextCommandAsync(command, cancellationToken),
